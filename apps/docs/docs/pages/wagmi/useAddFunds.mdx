...
Chain the QR code pins via EIP-681. Defaults to the first entry of `chains`, then the connected chain. Nothing is sent, so the connector does not need to be on this chain.
